...
- normalize legacy v2/v1 note/bomb/obstacle beat objects, plus late legacy v2.6 `_sliders`, into the current Boxing/Flow source families for authored chart emission
...
- synthetic legacy v2 conversion fixture proving truthful Boxing + Flow note/bomb/obstacle emission from legacy beat objects without inventing legacy burst/chain support
- synthetic late-legacy v2.6 staged fixture proving `_sliders` normalize into Flow `arc` emission through the existing conversion path
...
- late legacy v2.6 `_sliders` now normalize into the existing `sliders` family so Flow can emit truthful `arc` beats without widening other legacy support claims
- legacy burst-slider/chain parity remains unsupported, and non-Standard legacy characteristics stay out of scope for this converter slice
