...
static func run() -> Dictionary:
	...
	var create_result: Dictionary = runtime.create_new_song_package({
		"songId": "ab-song-seeded-smoke",
		"songName": "Seeded Smoke Song",
	...
	var root_charts: Array = Array(song_package.get("charts", []))
	var first_root_chart: Dictionary = Dictionary(root_charts[0]) if not root_charts.is_empty() else {}
	var seeded_linkage_passed := bool(create_result.get("ok", false)) \
		and String(song_package.get("songId", "")) == "ab-song-seeded-smoke" \
		and String(song_package.get("songName", "")) == "Seeded Smoke Song" \
		and String(song_package.get("packageVersion", "")) == "2.0.0" \
		and root_charts.size() == 1 \
		and String(first_root_chart.get("chartId", "")) == String(first_chart.get("chartId", "")) \
		and String(first_root_chart.get("difficulty", "")) == String(first_chart.get("difficulty", "")) \
	...
	var reload_passed := bool(load_result.get("ok", false)) \
		and String(reloaded_song_package.get("songId", "")) == "ab-song-seeded-smoke" \
